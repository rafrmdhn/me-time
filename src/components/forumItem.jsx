      ...
      <div>
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 -mt-1">
            <Link to={`/Diskusi/${user.idForum}`}>{user.judul}</Link>
          </h2>
        </div>
        <p className="text-gray-700 text-left">{user.namadepan} {user.namabelakang} • {user.createdAt}</p>
        <p className="mt-3 text-gray-700 text-sm text-left">{user.textforum}</p>
        <div className="mt-4 flex items-center">
          <div className="flex mr-2 text-gray-700 text-sm mr-8">
            <Link to={`/Diskusi/${user.idForum}`}className="flex">
              <svg
                fill="none"
                viewBox="0 0 24 24"
