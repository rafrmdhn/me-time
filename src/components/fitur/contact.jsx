...
const Contact = () => {
    ...
    const sendMessage = async (e) => {
        try {
            ...
            const response = await axios.post(`http://195.35.8.190:4000/api/messages`, {
                userId : parseInt(localStorageKey),
                message : message,
            })

            if(response) alert(response.data.msg);
    ...
    return (
        <section className="py-20 bg-[#F6F4EB]" id="contact">
            <div
                ...
                <div>
                    <h2 className="text-4xl font-bold text-left mr-20">Hubungi Kami Sekarang untuk Dukungan dan Bantuan yang Dipersonalisasi</h2>
                    <p className="max-w-sm mt-4 mb-4 text-left text-black">Ada yang ingin dikatakan? Kami di sini untuk membantu. Isi
                        formulir atau kirim email atau telepon.</p>
